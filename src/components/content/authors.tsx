...
export function Authors({
  ...
  return imageOnly ? (
    <Image
      src={authors[username as keyof typeof authors].image}
      alt={authors[username as keyof typeof authors].name}
      width={32}
      height={32}
    ...
    <Link
      href={`https://twitter.com/${authors[username as keyof typeof authors].twitter}`}
    ...
    >
      ...
      <div className="flex flex-col -space-y-0.5">
        <p className="font-semibold text-foreground max-md:text-sm">
          {authors[username as keyof typeof authors].name}
        </p>
        <p className="text-sm text-muted-foreground">
          @{authors[username as keyof typeof authors].twitter}
        </p>
      </div>
    </Link>
  );
